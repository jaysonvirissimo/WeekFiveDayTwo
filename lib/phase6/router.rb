module Phase6
  class Route
    ...
    def matches?(req)
      same_request = @http_method == req.request_method.downcase.intern
      same_request && !!(pattern =~ req.path)
    end
    ...
    def run(req, res)
      ...
      @controller_class.new(req, res, route_params).ivoke_action(action_name)
    end
  end
