...
describe Phase4::Session do
  ...
  let(:cook) { WEBrick::Cookie.new('ruby-on-tracks', { xyz: 'abc' }.to_json) }
  ...
  describe '#store_session' do
    context 'without cookies in request' do
      ...
      it "adds new cookie with 'ruby-on-tracks' name to response" do
        cookie = res.cookies.find { |c| c.name == 'ruby-on-tracks' }
    ...
    context 'with cookies in request' do
      before(:each) do
        cook = WEBrick::Cookie.new('ruby-on-tracks', { pho: 'soup' }.to_json)
        req.cookies << cook
      end
